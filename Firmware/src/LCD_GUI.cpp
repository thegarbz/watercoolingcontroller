/*****************************************************************************
  | File      	:	LCD_GUI.c
  | Author      :   Waveshare team - Modifed by Garbz
  ...
  ----------------
  |	This version:   V1.0 - Modifed by Garbz
  | Date        :   2019-05-22
  | Info        :   Basic version - Minor tweaks to improve speed of drawing
                    additional functions to handle Deci-degrees values and
                    correctly draw an input of e.g. 245 as 24.5.
...
******************************************************************************/
void GUI_DisChar(POINT Xpoint, POINT Ypoint, const char Ascii_Char,
                 sFONT* Font, COLOR Color_Background, COLOR Color_Foreground)
{
  ...
  if (Ascii_Char > '~') Char_Offset = (('~'+1) - ' ') * Font->Height * (Font->Width / 8 + (Font->Width % 8 ? 1 : 0));
  ...
  for (Page = 0; Page < Font->Height; Page ++ ) {
    for (Column = 0; Column < Font->Width; Column ++ ) {
      ...
      if (FONT_BACKGROUND == Color_Background) { //this process is to speed up the scan
        if (pgm_read_byte(ptr) & (0x80 >> (Column % 8))) GUI_DrawPoint(Xpoint + Column, Ypoint + Page, Color_Foreground, DOT_PIXEL_DFT, DOT_STYLE_DFT);
                ...
                sFONT* Font, COLOR Color_Background, COLOR Color_Foreground )
{
  ...
  //Fixes problems with 0 displaying and right justifies 
  if (Nummber == 0) 
  {
    Str_Array[0] = ' '; 
    Str_Array[1] = '0'; 
...
/******************************************************************************
  function:  Display the string in decimal 0.1 format
  parameter:
  Xstart           ：X coordinate
  Ystart           : Y coordinate
  Nummber          : The number displayed (in deci e.g. 25.4 = 254)
  Font             ：A structure pointer that displays a character size
  Color_Background : Select the background color of the English character
  Color_Foreground : Select the foreground color of the English character
******************************************************************************/
#define  ARRAY_LEN 255
void GUI_DisNum_Deci(POINT Xpoint, POINT Ypoint, int32_t Nummber,
                sFONT* Font, COLOR Color_Background, COLOR Color_Foreground )
{
  ...
  //Add decimals and split
  Str_Array[Str_Bit] = Str_Array[Str_Bit-1];
  Str_Array[Str_Bit-1] = '.';
...
******************************************************************************/
#define  ARRAY_LEN 255
void GUI_DisNum_Deci_Fast(POINT Xpoint, POINT Ypoint, int32_t Nummber, int32_t PrevNumber, 
                sFONT* Font, COLOR Color_Background, COLOR Color_Foreground )
{

  int16_t Num_Bit = 0, PrevNum_Bit = 0, Str_Bit = 0, offset = 0;
  uint8_t Str_Array[ARRAY_LEN] = {0}, Num_Array[ARRAY_LEN] = {0}, PrevNum_Array[ARRAY_LEN] = {0};
  uint8_t *pStr = Str_Array;
  ...
  while (PrevNumber) {
    PrevNum_Array[PrevNum_Bit] = PrevNumber % 10 + '0';
    PrevNum_Bit++;
    PrevNumber /= 10;
  }

  for (int i=Num_Bit; i > 0; i--) {
    if (Num_Array[i] == PrevNum_Array[i]) offset++;
  }
  offset--;
  
  //The string is inverted
  while (Num_Bit-offset > 0) { 
      Str_Array[Str_Bit] = Num_Array[Num_Bit - offset - 1];
      Str_Bit ++;
      Num_Bit --;
  }
  ...
  Xpoint = Xpoint + offset*Font->Width;
